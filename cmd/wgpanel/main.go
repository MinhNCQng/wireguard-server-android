...
func (a app) status(w http.ResponseWriter, r *http.Request) {
	...
	for _, line := range strings.Split(out, "\n") {
		if strings.HasPrefix(line, "backend=") {
			backend = strings.TrimPrefix(line, "backend=")
		}
	...
	writeJSON(w, status{Running: err == nil && !strings.Contains(out, "status=stopped"), WireGuard: out, DuckDNS: duck, LANEndpoint: localIPv4() + ":" + port, Endpoint: endpoint, DuckEnabled: duckErr == nil, LastRun: last, PublicIP: publicIP, ActivePeers: active, Backend: backend})
}
...
func (a app) forceDuckDNS(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "method", http.StatusMethodNotAllowed)
		return
	}
	if _, err := os.Stat(a.data("config", "duckdns.env")); err != nil {
		errorJSON(w, http.StatusConflict, fmt.Errorf("DuckDNS is not configured"))
		return
	}
	a.updateDuckDNS()
	writeJSON(w, map[string]string{"status": readTrim(a.data("runtime", "duckdns.status"))})
